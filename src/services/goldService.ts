  ...
  public static async getCompetitions(): Promise<GoldCompetition[]> {
    try {
      ...
      return data.map((c: any) => ({
        ...
        country: c.country || '',
        flag: c.flag || '',
        seasonsCount: Number(c.seasons_count) || 0,
  ...
  public static async getTeams(): Promise<GoldTeam[]> {
    try {
      ...
      return data.map((t: any) => ({
        id: t.id,
        name: t.name,
        shortName: t.short_name || (t.name ? t.name.substring(0, 3).toUpperCase() : ''),
        country: t.country || '',
        stadium: t.stadium || '',
        logo: t.logo || '',
        seasonStats: {
          ...
          xga: Number(t.xga) || 0,
          elo: Number(t.elo) || 0,
          formLast5: Array.isArray(t.form_last_5) ? t.form_last_5 : [],
        },
      }));
    } catch (e: any) {
  ...
  public static async getOddsExplorerRecords(filters?: { market?: string; season?: string; bookmaker?: string }): Promise<GoldOddsRecord[]> {
    try {
      ...
      return data.map((o: any) => ({
        ...
        openingOdds: Number(o.opening_odds) || 0,
        closingOdds: Number(o.closing_odds) || 0,
