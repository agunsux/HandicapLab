...
export function TeamMatchesTable({ matches, currentTeamName }: TeamMatchesTableProps) {
  ...
  if (!mounted) {
    return (
      <div className="flex items-center justify-center p-12 border border-slate-800 bg-slate-900/40 rounded-xl">
        <span className="text-xs font-mono text-slate-500 animate-pulse">Initializing Data Feed...</span>
      </div>
    );
  }
  ...
  if (matches.length === 0) {
    return (
      <div className="text-center py-12 border border-slate-800 bg-slate-900/20 rounded-xl font-mono text-xs text-slate-500">
        No active upcoming matches scheduled for {currentTeamName}.
      </div>
    );
  }
      ...
      <Table>
        <TableHeader className="bg-slate-950/80 border-b border-slate-800">
          <TableRow className="hover:bg-transparent border-slate-800">
            ...
            <TableHead className="text-slate-400 font-mono text-xs">Fixture & Kickoff</TableHead>
            <TableHead className="text-slate-400 font-mono text-xs text-center">AH Line</TableHead>
            ...
            <TableHead className="text-slate-400 font-mono text-xs text-center">Home/Draw/Away Prob</TableHead>
        ...
        <div className="absolute bottom-0 inset-x-0 h-40 bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent flex flex-col justify-end items-center pb-6 px-4 text-center z-10 pointer-events-auto">
          <div className="bg-slate-900/95 border border-slate-800 p-4 rounded-xl max-w-md shadow-2xl backdrop-blur-md space-y-2">
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Viewing 3 free matches. Access ensembled goal models, handicap edges, and Kelly stakes for all of {currentTeamName}&apos;s fixtures.
            </p>
