...
export function PaperTradingDashboard({ initialTrades }: PaperTradingDashboardProps) {
  ...
  // Sort trades chronologically to build equity curve & compute drawdown
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
      ...
      <div className="bg-slate-950/60 border border-slate-800 p-4 rounded-xl text-xs font-mono text-slate-400 flex items-start gap-3">
        ...
        <div className="space-y-1">
          ...
          <p className="leading-relaxed">
            This dashboard displays simulated paper trading models. This is NOT a gambling platform. No real currency is used. The engine executes simulated trades automatically only if: **Model Confidence is &gt;= 70%** AND **Edge Score is &gt;= EV threshold** AND **Expected Value &gt; 0**. Trades are never placed based on confidence alone.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl space-y-2 col-span-2">
          <span className="text-[10px] font-mono text-slate-500 font-bold uppercase tracking-wider block">Telemetry Provenance</span>
          <div className="text-xs font-mono text-slate-300 mt-2">
            Picks: <span className="text-white font-bold">{initialTrades.length}</span> (Settled: <span className="text-white font-bold">{settledTrades.length}</span>)
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Simulation Equity Curve (Units)</h2>
        ...
        <div className="flex flex-wrap gap-1.5">
          {(['all', 'pending', 'won', 'lost', 'void'] as const).map((tab) => (
            <button
              ...
              className={`px-3 py-1 rounded text-[11px] font-mono font-bold transition-all uppercase ${
                filter === tab
