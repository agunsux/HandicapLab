...
async function handleGenerateSignals(request: Request) {
  ...
  if (shouldDeferProviderWork(request)) {
    const eventId = await enqueueEgressJob({ job: 'generate_signals', scope: utcHourKey() });
    return NextResponse.json({ success: true, mode: 'worker', job: 'generate_signals', enqueued: eventId });
  }
      ...
      if (!oddsList) {
        ...
        for (const marketType of ['ML', 'AH', 'OU'] as const) {
          try {
            ...
            if (marketType === 'ML') {
              // Moneyline is deprecated and out of scope for HandicapLab (EPIC 61).
              continue;
            } else if (marketType === 'AH') {
              const spreadsMarket = pinnacle.markets.find((m: any) => m.key === 'spreads');
