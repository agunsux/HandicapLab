...
export async function GET(request: Request) {
  try {
    ...
    const result = await FootballIntelligenceService.getMatchExplanations(matchId);
    if (!result) {
      return ApiHelper.response(false, null, `No explainability data found for match ${matchId}.`, 404);
    }
