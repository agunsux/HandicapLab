      ...
      if (!options.allowProviderCalls) {
        ...
        if (published && published.length > 0) {
          const picks: DailyPickRecord[] = published.map((s) => ({
            ...
            line: s.line ?? 0,
            predictionTimestampUtc: s.predictionTimestampUtc || s.lastReconciledUtc || new Date().toISOString(),
            oddsTimestampUtc: s.oddsTimestampUtc || s.lastReconciledUtc || new Date().toISOString(),
            modelVersion: s.providerProvenance?.modelVersion || 'dixon-coles-v1.0',
            dataVersion: 'canonical-published',
            providerSources: s.providerProvenance || { fixtures: 'api-football-pro', odds: 'oddspapi-pinnacle', statistics: 'apifootball' },
            modelProbability: s.modelProbability ?? 0.5,
            marketProbability: s.marketProbability ?? 0.5,
            fairOdds: s.fairOdds ?? (s.modelProbability > 0 ? Number((1 / s.modelProbability).toFixed(2)) : 2.0),
            marketOdds: s.currentOdds ?? 2.0,
            edge: s.edge ?? 0,
            expectedValue: s.expectedValue ?? 0,
            confidence: s.confidence ?? 50,
            strengthLevel: s.strengthLevel || (s.confidence >= 60 ? 'STRONG' : s.confidence >= 50 ? 'MODERATE' : s.confidence >= 40 ? 'WEAK' : 'VERY_WEAK'),
            signalColor: s.signalColor || (s.confidence >= 60 ? 'green' : s.confidence >= 50 ? 'yellow' : s.confidence >= 40 ? 'orange' : 'red'),
            publishState: s.publishState || 'PUBLISHED',
            confidenceDisclaimer: s.confidenceDisclaimer || 'Confidence represents multi-factor model robustness and sample sufficiency, separate from win probability.',
            freshnessText: s.freshnessText || 'fresh',
            validationStatus: s.validityStatus === 'VALID' ? 'VALIDATED_EDGE' : 'PROVISIONAL_EDGE',
            dataQuality: 95,
            providerHealth: 'HEALTHY',
            status: 'ACTIVE',
            apiFootballFixtureTimestamp: s.lastReconciledUtc || new Date().toISOString(),
            oddsPapiSnapshotTimestamp: s.oddsTimestampUtc || s.lastReconciledUtc || new Date().toISOString(),
          }));
