/**
 * SUPER EPIC 31B.5 — Expanded Statistical Validator
 * Calculates: Log Loss, Brier, ECE, MCE, F1, Accuracy, Precision, Recall,
 * ROC AUC, PR AUC, Sortino Ratio, Sharpe Ratio, Decile lifts, BH corrections.
 */
...
} from '../../lib/epic31b/types';
import { DriftDetector } from './drift-detector';

export class StatisticalValidator {
  /**
   * Run all statistical validators.
   */
  static validate(outcomes: ReplayOutcome[]): StatisticalValidatorOutput {
    ...
    const driftDetected = drift.driftStatus === 'DRIFT_DETECTED';
  ...
  static computeMetrics(outcomes: ReplayOutcome[]): ReplayMetrics {
    ...
    const sortinoRatio = downsideDeviation > 0 ? (meanReturn / downsideDeviation) * Math.sqrt(outcomes.length) : null;
    ...
    for (const o of outcomes) {
      const pred = o.predictedProbability >= 0.5 ? 1 : 0;
      const act = o.actualResult === 1 ? 1 : 0;
      if (pred === 1 && act === 1) tp++;
    ...
    for (let i = 0; i < sortedRoc.length - 1; i++) {
      rocauc += 0.5 * (sortedRoc[i].tpr + sortedRoc[i + 1].tpr) * (sortedRoc[i + 1].fpr - sortedRoc[i].fpr);
    }

    const prPoints = this.computePrPoints(outcomes);
    let prauc = 0;
    const sortedPr = [...prPoints].sort((a, b) => a.recall - b.recall);
    for (let i = 0; i < sortedPr.length - 1; i++) {
      prauc += 0.5 * (sortedPr[i].precision + sortedPr[i + 1].precision) * (sortedPr[i + 1].recall - sortedPr[i].recall);
    }
    ...
    return {
      ...
      sortinoRatio: sortinoRatio ? Math.round(sortinoRatio * 10000) / 10000 : null,
      ...
      prauc: Math.round(prauc * 10000) / 10000,
  ...
  static buildLeagueValidationResult(
    ...
    const statOutput = this.validate(outcomes);
    const status = validationReport.validFixtures > 0 && statOutput.metrics.totalPredictions > 0 ? 'PASS' : 'FAIL';

    return {
      leagueId: leagueId as any,
      leagueName: (leagueId === '39' ? 'EPL' : leagueId) as any,
      status,
      evidence: `${statOutput.metrics.totalPredictions} predictions. ROI: ${statOutput.metrics.roi}%, CLV: ${statOutput.metrics.avgClv}%, ECE: ${statOutput.metrics.ece}%`,
      metrics: statOutput.metrics,
      confidenceIntervals: statOutput.confidenceIntervals,
      calibrationQuality: statOutput.calibrationQuality,
      statisticalConfidence: statOutput.statisticalConfidence,
      driftDetected: statOutput.driftDetected,
    };
  }
}
