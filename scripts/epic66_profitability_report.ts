...
interface MarketDiscoveryRanking {
  market: 'AH' | 'OU' | 'BTTS';
  dimension: string;
  identifier: string;
  leagueId: string;
...
export function generateReports() {
  console.log('Generating EPIC-66 Forensic Markdown Reports...');

  const discoveryJsonPath = path.resolve('data/reports/epic66_market_discovery.json');
  const walkforwardJsonPath = path.resolve('data/reports/epic66_walkforward_report.json');
  const coverageJsonPath = path.resolve('data/reports/epic66_coverage_matrix.json');

  const discoveryData: { evaluated_cells: number; rankings: MarketDiscoveryRanking[] } = JSON.parse(
    fs.readFileSync(discoveryJsonPath, 'utf-8')
  );
  const walkforwardData: { total_strategies: number; strategies: WalkForwardStrategy[] } = JSON.parse(
    fs.readFileSync(walkforwardJsonPath, 'utf-8')
  );
  const coverageData: { total_leagues: number; cells: any[] } = JSON.parse(
    fs.readFileSync(coverageJsonPath, 'utf-8')
  );

  // 1. Generate data/reports/epic66_market_discovery.md
  let discoveryMd = `# EPIC 66 — Baseline Market Discovery Report\n\n`;
  discoveryMd += `**Evaluated Slices**: ${discoveryData.evaluated_cells}\n`;
  discoveryMd += `**European Universe**: Top 5 Leagues (11,642 canonical matches, 100% Pinnacle closing lines)\n`;
  discoveryMd += `**Global Universe**: 30 Leagues (17,738 completed fixtures)\n\n`;

  discoveryMd += `## Top Baseline Market Slices (Ranked by ROI)\n\n`;
  discoveryMd += `| Rank | Identifier | Market | Side | Bets | Hit Rate | ROI % | Max DD | p-value | FDR q-value | Tier |\n`;
  discoveryMd += `| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n`;

  discoveryData.rankings.slice(0, 20).forEach((r, idx) => {
    discoveryMd += `| ${idx + 1} | \`${r.identifier}\` | ${r.market} | ${r.side} | ${r.bets} | ${r.hitRatePct}% | ${r.roiPct > 0 ? '+' : ''}${r.roiPct}% | ${r.maxDrawdown}u | ${r.pValue.toFixed(4)} | ${r.fdrQValue !== undefined ? r.fdrQValue.toFixed(4) : 'N/A'} | **${r.tier}** |\n`;
  });

  discoveryMd += `\n## Bottom Baseline Market Slices (Underperforming / Heavy Losers)\n\n`;
  discoveryMd += `| Identifier | Market | Side | Bets | Hit Rate | ROI % | Max DD | Losing Streak | Tier |\n`;
  discoveryMd += `| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n`;

  discoveryData.rankings.slice(-10).forEach((r) => {
    discoveryMd += `| \`${r.identifier}\` | ${r.market} | ${r.side} | ${r.bets} | ${r.hitRatePct}% | ${r.roiPct}% | ${r.maxDrawdown}u | ${r.maxLosingStreak} | **${r.tier}** |\n`;
  });

  discoveryMd += `\n## Global League BTTS Frequency Spectrum (30 Leagues)\n\n`;
  discoveryMd += `| League Code | Total Fixtures | BTTS Rate | Baseline ROI @ 1.90 | Empirical Status |\n`;
  discoveryMd += `| :--- | :--- | :--- | :--- | :--- |\n`;

  const bttsRankings = discoveryData.rankings.filter((r) => r.market === 'BTTS');
  bttsRankings.forEach((b) => {
    discoveryMd += `| \`${b.leagueId}\` | ${b.bets} | ${b.hitRatePct}% | ${b.roiPct > 0 ? '+' : ''}${b.roiPct}% | **${b.tier}** |\n`;
  });

  const discoveryMdPath = path.resolve('data/reports/epic66_market_discovery.md');
  fs.writeFileSync(discoveryMdPath, discoveryMd, 'utf-8');
  console.log(`Wrote ${discoveryMdPath}`);

  // 2. Generate data/reports/epic66_walkforward_report.md
  let wfMd = `# EPIC 66 — Walk-Forward Model Evaluation vs Pinnacle Baseline\n\n`;
  wfMd += `**Total Strategies Tested**: ${walkforwardData.total_strategies}\n`;
  wfMd += `**In-Sample Period**: Season 2024/2025 (50% historical slice)\n`;
  wfMd += `**Out-of-Sample Period**: Season 2025/2026 (50% chronological forward test)\n`;
  wfMd += `**Model**: Dixon-Coles Bivariate Poisson Expectancy vs Pinnacle Closing Odds\n\n`;

  wfMd += `## Walk-Forward Model Comparison Table\n\n`;
  wfMd += `| Strategy | Baseline ROI | IS Bets | IS ROI | OOS Bets | OOS ROI | Model N | Model ROI | Alpha vs Mkt | Mean CLV | Tier |\n`;
  wfMd += `| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n`;

  walkforwardData.strategies
    .filter((s) => s.totalModelBets >= 20 || s.totalModelRoiPct > 0)
    .sort((a, b) => b.totalModelRoiPct - a.totalModelRoiPct)
    .forEach((s) => {
      wfMd += `| \`${s.strategyName}\` | ${s.baselineRoiPct}% | ${s.inSampleBets} | ${s.inSampleRoiPct}% | ${s.outOfSampleBets} | ${s.outOfSampleRoiPct}% | ${s.totalModelBets} | **${s.totalModelRoiPct > 0 ? '+' : ''}${s.totalModelRoiPct}%** | ${s.alphaVsMarketPct > 0 ? '+' : ''}${s.alphaVsMarketPct}% | **+${s.modelClvMeanPct}%** | **${s.tier}** |\n`;
    });

  const wfMdPath = path.resolve('data/reports/epic66_walkforward_report.md');
  fs.writeFileSync(wfMdPath, wfMd, 'utf-8');
  console.log(`Wrote ${wfMdPath}`);

  // 3. Generate docs/EPIC66_GLOBAL_MARKET_PROFITABILITY_REPORT.md
  let masterDoc = `# EPIC 66 — Global Historical Market Discovery & Profitability Report
...
`;
  ...
  const masterDocPath = path.resolve('docs/EPIC66_GLOBAL_MARKET_PROFITABILITY_REPORT.md');
  fs.writeFileSync(masterDocPath, masterDoc, 'utf-8');
  console.log(`Wrote ${masterDocPath}`);

  console.log('\nAll EPIC-66 Forensic Reports Generated Successfully.');
}
